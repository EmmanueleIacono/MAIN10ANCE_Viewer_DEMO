<template>
  <div v-if="store.stateBIM.schedeRisultatiVisibile" id="formDB">
    ...
    <Details v-if="state.schedeAnagrafica && state.schedeAnagrafica.length" summary="Scheda anagrafica">
      <BIMScheda v-for="scheda in state.schedeAnagrafica" :key="scheda['Codice scheda anagrafica']" :scheda="scheda" tipo="anagrafica" />
    </Details>
    <Details v-if="state.schedeControllo && state.schedeControllo.length" summary="Schede di controllo">
      <BIMScheda v-for="scheda in state.schedeControllo" :key="scheda['Codice scheda controllo']" :scheda="scheda" tipo="controllo" />
    </Details>
    <Details v-if="state.schedeManOrd && state.schedeManOrd.length" summary="Schede di manutenzione ordinaria">
      <BIMScheda v-for="scheda in state.schedeManOrd" :key="scheda['Codice scheda manuetenzione regolare']" :scheda="scheda" tipo="intervento" />
    </Details>
    <Details v-if="state.schedeManCorr && state.schedeManCorr.length" summary="Schede di manutenzione correttiva">
      <BIMScheda v-for="scheda in state.schedeManCorr" :key="scheda['Codice scheda manutenzione correttiva']" :scheda="scheda" tipo="intervento" />
    </Details>
    <Details v-if="state.schedeManStr && state.schedeManStr.length" summary="Schede di manutenzione straordinaria">
      <BIMScheda v-for="scheda in state.schedeManStr" :key="scheda['Codice scheda manutenzione straordinaria']" :scheda="scheda" tipo="intervento" />
    </Details>
    <Details v-if="state.schedeRestauro && state.schedeRestauro.length" summary="Schede di restauro">
      <BIMScheda v-for="scheda in state.schedeRestauro" :key="scheda['Codice scheda restauro']" :scheda="scheda" tipo="intervento" />
    </Details>
  </div>
</template>
