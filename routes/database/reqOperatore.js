...
async function leggiColonneTabella(nomeTab) {
    try {
        ...
        const result = await poolM10a.query(`SELECT * FROM information_schema.columns WHERE table_schema = $1 AND table_name = $2;`, [data_schema, nomeTab]);
        return result.rows;
    }
    catch(e) {
        return [];
...
async function leggiTabellaDB(nomeTab) {
    try {
        const result = await poolM10a.query(`SELECT * FROM ${qualifiedName(data_schema, nomeTab)} ORDER BY "id_main10ance";`);
        return result.rows;
    }
    catch(e) {
        return [];
...
async function leggiTabellaGlossario() {
    try {
        const result = await poolM10a.query(`SELECT * FROM ${utility_schema}."glossario" ORDER BY regexp_replace("id_gloss", '\\D', '', 'g');`);
        return result.rows;
    }
    catch(e) {
        return [];
...
async function leggiTabellaGlossarioDegradi() {
    try {
        const result = await poolM10a.query(`SELECT * FROM ${utility_schema}."glossario" WHERE ("gloss_ty"='danno' OR "gloss_ty"='alterazione' OR "gloss_ty"='degrado' OR "gloss_ty"='nessuno') ORDER BY regexp_replace("id_gloss", '\\D', '', 'g');`);
        return result.rows;
    }
    catch(e) {
        console.log(e);
...
async function leggiEnum(nomeEnum) {
    try {
        const result = await poolM10a.query(`SELECT unnest(enum_range(null::${quoteIdentifier(data_schema)}.${quoteIdentifier(nomeEnum)}));`);
        return result.rows;
    }
    catch(e) {
        return [];
...
async function leggiEnumServizio(nomeEnum) {
    try {
        const result = await poolM10a.query(`SELECT unnest(enum_range(null::${quoteIdentifier(utility_schema)}.${quoteIdentifier(nomeEnum)}));`);
        return result.rows;
    }
    catch(e) {
        return [];
...
async function leggiTabelleLOD3e4() {
    try {
        const results = await poolM10a.query(`SELECT "entità_db_m10a" AS "tabella", "nome_esteso" AS "alias" FROM ${utility_schema}."lod" WHERE "LOD" = 3 OR "LOD" = 4 ORDER BY "alias";`);
        return results.rows;
    }
    catch(e) {
        return [];
        ...
        await withTransaction(async (tx) => {
            ...
            case stringaContr: {
                if (dati.nuovo_record) {
                    ...
                    await tx.query(`UPDATE ${data_schema}."${stringaContr}" SET "cl_racc" = ($2), "st_cons" = ($3), "liv_urg" = ($4), "estensione" = ($5), "strumentaz" = ($6), "commenti" = ($7), "docs" = ($8), "costo" = ($9), "ore" = ($10), "data_con" = ($11), "data_ultima_mod" = ($12), "autore_ultima_mod" = ($13), "id_main10ance" = ($14), "eseguito" = ($15) WHERE "id_contr" = ($1) AND "ambito" = ($16);`, arrayUpdateContr);
                }
            ...
            case stringaManReg: {
                if (dati.nuovo_record) {
                    ...
                    await tx.query(`UPDATE ${data_schema}."${stringaManReg}" SET "materiale" = ($2), "strumentaz" = ($3), "commenti" = ($4), "docs" = ($5), "costo" = ($6), "ore" = ($7), "data_ese" = ($8), "data_ultima_mod" = ($9), "autore_ultima_mod" = ($10), "id_main10ance" = ($11), "eseguito" = ($12) WHERE "id_mn_reg" = ($1);`, arrayUpdateManReg);
                }
            ...
            case stringaManCorr: {
                if (dati.nuovo_record) {
                    ...
                    await tx.query(`UPDATE ${data_schema}."${stringaManCorr}" SET "materiale" = ($2), "strumentaz" = ($3), "commenti" = ($4), "docs" = ($5), "costo" = ($6), "ore" = ($7), "data_ese" = ($8), "data_ultima_mod" = ($9), "autore_ultima_mod" = ($10), "id_main10ance" = ($11), "eseguito" = ($12) WHERE "id_mn_gu" = ($1);`, arrayUpdateManCorr);
                }
...
async function interrogaAnagraficaLOD4(id, ambito) {
    try {
        const result = await poolM10a.query(`SELECT sa.autore_ultima_mod AS "Operatore", sa.descrizione_sistema AS "Descrizione sistema", sa.descrizione_subsistema AS "Descrizione subsistema", sa.tecnica_costruttiva AS "Tecnica costruttiva", sa.dimensioni AS "Dimensioni", sa.materiale AS "Materiale/i", sa.epoca AS "Epoca", sa.ispezionabilità AS "Ispezionabilità", sa.fonti AS "Fonti" FROM ${data_schema}.scheda_anagrafica AS sa WHERE sa.id_main10ance = $1 AND sa.ambito = $2;`, [id, ambito]);
        return result.rows;
    }
    catch(e) {
        console.log(e);
...
async function interrogaAnagraficaStatua(id, ambito) {
    try {
        const result_anagr = await poolM10a.query(`SELECT sa.autore_ultima_mod AS "Operatore",
        ...
        const result_st = await poolM10a.query(`SELECT nome AS "Soggetto statua", codice AS "Codice statua"
...
async function interrogaAnagraficaCopertura(id, ambito) {
    try {
        ...
        const result_cop = await poolM10a.query(`SELECT codice AS "Codice copertura"
...
async function interrogaAnagraficaManufatto(id) {
    try {
        const result = await poolM10a.query(`SELECT sa.autore_ultima_mod AS "Operatore", sa.definizione AS "Definizione", sa.epoca AS "Epoca", sa.autore AS "Autore", sa.descrizione AS "Descrizione", sa.materiale AS "Materiale/i", sa.tecniche AS "Tecniche", sa.documenti AS "Documenti", sa.iter_autorizzativo AS "Iter autorizzativo" FROM ${utility_schema}.anagrafica_manufatto AS sa WHERE sa.id_main10ance = $1 ORDER BY sa.id_anagr DESC;`, [id]);
        return result.rows;
    }
    catch(e) {
        console.log(e);
...
async function interrogaAnagraficaDettaglio(id) {
    try {
        const result = await poolM10a.query(`SELECT sa.autore_ultima_mod AS "Operatore", sa.definizione AS "Definizione", sa.descrizione AS "Descrizione", sa.materiale AS "Materiale/i", sa.tecniche AS "Tecniche", sa.epoca AS "Epoca", sa.documenti AS "Documenti", sa.autore AS "Autore", sa.data AS "Data" FROM ${utility_schema}.anagrafica_dettaglio AS sa WHERE sa.id_main10ance = $1 ORDER BY sa.id_anagr DESC;`, [id]);
        return result.rows;
    }
    catch(e) {
        console.log(e);
...
async function leggiVistaDB(nome_vista, utility = false) {
    ...
    try {
        const results = await poolM10a.query(`SELECT * FROM ${qualifiedName(schema, nome_vista)};`);
        return results.rows;
    }
    catch(e) {
        console.log(e);
...
async function leggiListaClassiOggetto() {
    try {
        const results = await poolM10a.query(`SELECT DISTINCT cl_ogg FROM ${utility_schema}."classificazione_elementi";`);
        return results.rows;
    }
    catch(e) {
        console.log(e);
...
async function leggiListaElementi() {
    try {
        const results = await poolM10a.query(`SELECT cl_ogg, elemento_sigla, elemento_nome FROM ${utility_schema}."classificazione_elementi";`);
        return results.rows;
    }
    catch(e) {
        console.log(e);
...
async function caricaDocumento(dati, files, ambito, utente) {
    ...
    try {
        // insert DB
        await withTransaction(async (tx) => {
            await tx.query(insertSQL, insertVals);
...
async function leggiIdDocEsistenti(ambito) {
    try {
        const results = await poolM10a.query(`SELECT id_doc FROM ${data_schema}.documenti WHERE ambito LIKE $1 ORDER BY id_doc`, [ambito]);
        return results.rows;
    } catch (err) {
        console.error(err);
        return [];
